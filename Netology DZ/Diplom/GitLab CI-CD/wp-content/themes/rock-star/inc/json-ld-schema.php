 ...
 * Adopted from Dimox
 *
 * @since Rock_Star 1.6.6
 */
class Rockstar_Json_Ld_Schema {
	private $position = 0;
	...
	function rock_star_custom_breadcrumbs_json_ld( $show_on_home ) {
		/* === OPTIONS === */
		$text['home']     = __( 'Home', 'rock-star' ); // text for the 'Home' link
		$text['category'] = __( 'Archive for ', 'rock-star' ); // text for a category page
		$text['search']   = __( 'Search results for: ', 'rock-star' ); // text for a search results page
		$text['tag']      = __( 'Posts tagged ', 'rock-star' ); // text for a tag page
		$text['author']   = __( 'View all posts by ', 'rock-star' ); // text for an author page
		$text['404']      = __( 'Error 404', 'rock-star' ); // text for the 404 page
	...
	function json_ld_breadcrumbs() {
		...
		if ( isset( $options['breadcrumb_option'] ) && $options['breadcrumb_option'] ) {
			$show_on_home = ( isset( $options['breadcrumb_on_homepage'] ) && $options['breadcrumb_on_homepage'] ) ? 1 : 0;

			$breadcrumb = $this->rock_star_custom_breadcrumbs_json_ld( $show_on_home );
			$json       = '';
